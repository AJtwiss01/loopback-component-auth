...
module.exports = function componentLoader(app, spec) {
  ...
  const models = _.defaults({}, spec.models || {}, {
    ...
    accessTokenModel: app.models.AccessToken,
  });
  ...
  Object
    ...
    .forEach((model) => {
      if (_.isString(models[model])) {
        models[model] = app.getModel(models[model]);
      }
    });
  ...
  function setupAuthProvider(name, options) {
    ...
    const link = utils.link(providerOptions);
    ...
    if (link) {
      const providerAuthPath = utils.authPath(name, link, providerOptions);
      const providerAuthHTTPMethod = utils.authHTTPMethod(providerOptions);
      const providerCallbackPath = utils.callbackPath(name, link, providerOptions);
      ...
      app.middlewareFromConfig(loopback.cookieParser, {
        ...
        paths: _.uniq([providerAuthPath, providerCallbackPath]),
      });
      ...
      app.middleware('auth:after', providerAuthPath, (req, res, next) => {
        ...
        if (!requestPath.endsWith(providerAuthPath.replace(/\/$/, ''))) {
          return next();
        }
      ...
      app.middlewareFromConfig(loopback.token, {
        ...
        paths: [providerCallbackPath],
      });

      // clear cookie on first access
      app.middleware('auth:after', providerCallbackPath, (req, res, next) => {
        res.clearCookie(cookie.name, {
          path: cookie.options.path,
        });
        next();
