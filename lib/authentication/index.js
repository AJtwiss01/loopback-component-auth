...
module.exports = function setupCustomAuthType(app, appAuthDir, models, authType, name, options) {
  ...
  function successRedirect(req) {
    ...
    return providerOptions.successRedirect || `/account/${(link ? 'link' : 'login')}-success/${name}`;
  }

  const failureRedirect = providerOptions.failureRedirect || (link ? `/account/authorize/${name}` : '/account/login');
    ...
    passport.authenticate(name, authenticateOptions, (err, user, info) => {
      ...
      if (!user) {
        ...
        const redirectTo = url.format(
          _.merge(url.parse(failureRedirect), {
            query: {
              error_code: 401,
