...
module.exports = function setupCustomAuthType(app, appAuthDir, models, authType, name, options) {
  ...
  function successRedirect(req) {
    ...
    if (link) {
      return `/account/authorize/link-success/${name}?state=success`;
    }
    return '/account?state=success';
  }

  const failureRedirect = providerOptions.failureRedirect || (link ?
    `/account/authorize/link-failure/${name}?state=failure` :
    '/account/login'
  );
